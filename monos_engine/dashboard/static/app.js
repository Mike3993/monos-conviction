...
function switchTab(name) {
  ...
  // When switching to decisions tab, auto-load
  if (name === "decisions") {
    loadDecisionReport();
  }
...
let lastDecReport = null;

function setDecStatus(msg, ok) {
  const el = $("dec-status");
  el.textContent = msg;
  el.className = "status " + (ok ? "ok" : "error");
  el.classList.remove("hidden");
...
async function loadDecisionReport() {
  try {
    const res = await (await fetch("/api/decision-report")).json();
    if (!res.ok) { setDecStatus("Error: " + (res.error || "unknown"), false); return; }
    lastDecReport = res;
    renderDecSummary(res);
    renderDecImpact(res.translation_impact);
    renderDecMode(res.by_mode);
    renderDecRule(res.by_rule);
    renderDecStructure(res.by_structure);
    if (res.total > 0) {
      setDecStatus(`Report loaded: ${res.total} decisions`, true);
    }
  } catch (e) {
    setDecStatus("Failed: " + e.message, false);
  }
}

function renderDecSummary(data) {
  if (!data || data.total === 0) {
    $("dec-summary-cards").innerHTML = '<p class="placeholder-text">Run batches and close trades to populate the decision report.</p>';
    hide("dec-impact-section"); hide("dec-mode-section"); hide("dec-rule-section"); hide("dec-struct-section");
    return;
  }
  const o = data.overall || {};
  const imp = data.translation_impact || {};
  const cards = [
    ["Total Decisions", data.total],
    ["Win Rate", fmt(o.win_rate, 1) + "%"],
    ["Avg Opt Return", o.avg_opt_return != null ? fmt(o.avg_opt_return, 2) + "%" : "—"],
    ["Boosted Trades", imp.boosted_count || 0],
    ["Translation Delta", imp.translation_delta != null ? fmt(imp.translation_delta, 2) + "%" : "—"],
    ["Structure Changes", imp.structure_changes || 0],
  ];
  let html = "";
  cards.forEach(([label, value]) => {
    let cls = "";
    ...
    if (s.includes("%")) {
      const n = parseFloat(s);
      if (!isNaN(n) && label !== "Win Rate") cls = pctClass(n);
    }
    html += `<div class="card"><div class="label">${label}</div><div class="value ${cls}">${value}</div></div>`;
  });
  $("dec-summary-cards").innerHTML = html;
}

function renderDecImpact(imp) {
  if (!imp || !imp.total_decisions) { hide("dec-impact-section"); return; }
  const cards = [
    ["Boosted WR", fmt(imp.boosted_wr, 1) + "%"],
    ["Boosted Avg", imp.boosted_avg != null ? fmt(imp.boosted_avg, 2) + "%" : "—"],
    ["Unboosted WR", fmt(imp.unboosted_wr, 1) + "%"],
    ["Unboosted Avg", imp.unboosted_avg != null ? fmt(imp.unboosted_avg, 2) + "%" : "—"],
    ["Delta", imp.translation_delta != null ? fmt(imp.translation_delta, 2) + "%" : "—"],
    ["Avg Boost", imp.avg_boost != null ? fmt(imp.avg_boost, 4) : "—"],
  ];
  let html = "";
  cards.forEach(([label, value]) => {
    let cls = "";
    if (label === "Delta") cls = parseFloat(String(value)) >= 0 ? "pos" : "neg";
    html += `<div class="card"><div class="label">${label}</div><div class="value ${cls}">${value}</div></div>`;
  });
  $("dec-impact-cards").innerHTML = html;
  show("dec-impact-section");
}

function renderDecMode(byMode) {
  if (!byMode || !Object.keys(byMode).length) { hide("dec-mode-section"); return; }
  let html = "";
  Object.entries(byMode).sort((a,b) => (b[1].avg_return||0) - (a[1].avg_return||0)).forEach(([mode, d]) => {
    html += `<tr>
      <td><strong>${mode}</strong></td>
      <td>${d.trades}</td>
      <td>${fmt(d.win_rate, 1)}%</td>
      <td class="${pctClass(d.avg_return)}">${d.avg_return != null ? fmt(d.avg_return, 2) + '%' : '—'}</td>
    </tr>`;
  });
  $("dec-mode-body").innerHTML = html;
  show("dec-mode-section");
}

function renderDecRule(byRule) {
  if (!byRule || !Object.keys(byRule).length) { hide("dec-rule-section"); return; }
  let html = "";
  Object.entries(byRule).sort((a,b) => b[1].trades - a[1].trades).forEach(([rid, d]) => {
    html += `<tr>
  ...
  $("dec-rule-body").innerHTML = html;
  show("dec-rule-section");
}

function renderDecStructure(byStruct) {
  if (!byStruct || !Object.keys(byStruct).length) { hide("dec-struct-section"); return; }
  let html = "";
  Object.entries(byStruct).forEach(([s, d]) => {
    html += `<tr>
      <td><strong>${s}</strong></td>
      <td>${d.original_count}</td>
      <td>${fmt(d.original_wr, 1)}%</td>
      <td class="${pctClass(d.original_avg)}">${d.original_avg != null ? fmt(d.original_avg, 2) + '%' : '—'}</td>
      <td>${d.translated_count}</td>
      <td>${fmt(d.translated_wr, 1)}%</td>
      <td class="${pctClass(d.translated_avg)}">${d.translated_avg != null ? fmt(d.translated_avg, 2) + '%' : '—'}</td>
    </tr>`;
  });
  $("dec-struct-body").innerHTML = html;
  show("dec-struct-section");
}

function copyDecisionReport() {
  if (!lastDecReport || !lastDecReport.formatted) {
    setDecStatus("No report to copy", false);
    return;
  }
  navigator.clipboard.writeText(lastDecReport.formatted);
  setDecStatus("Decision report copied to clipboard", true);
}
